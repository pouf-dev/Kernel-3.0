...
import json
import random
from Crypto.Util.number import long_to_bytes, bytes_to_long#rajouté
...
class Encryptor(object):
    ...
    def encryptBlock(self, block):
        key = list(self.generateKey())# la clé change à chaque appel d'encyptBlock
        l = list(block[:8])
        r = list(block[8:])
    ...
    def decrypt(self, plaintext, ctr):
        while len(plaintext)%16:
            plaintext += b'\0'
...
"""
f=open("sale.xlsx.enc", 'rb')
s=f.read()
f.close()
ctr=bytes_to_long(s[:16])
plaintext=s[16:]
recup = Encryptor("pouetpouetpouet").decrypt(plaintext,ctr)
f=open("recup.xlsx","wb")
f.write(recup)
f.close()
"""
